...
const EditFeeCancel = () => {
  ...
  const id = params.id_fee_cancels; // Pastikan nama ini sesuai dengan nama folder

  useEffect(() => {
    console.log('Params:', params); // Debugging log

    if (id) {
      const fetchFeeCancel = async () => {
        setLoading(true);
        try {
          const response = await axios.get(`http://localhost:8000/api/feecancel/show/${id}`);
          const { fee, user_id, nama_lengkap } = response.data;
          setFee(fee.toString());
          setUserId(user_id.toString());
          setNamaLengkap(nama_lengkap);
        } catch (error) {
          setError('Error fetching fee cancel data');
          console.error('Error fetching fee cancel data:', error);
        } finally {
          setLoading(false);
        }
      };
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setError(null);
    try {
      const response = await axios.post(`http://localhost:8000/api/feecancel/update/${id}`, {
        fee: feeInt,
        user_id: userIdInt,
      ...
      if (axios.isAxiosError(error) && error.response) {
        console.error('Error updating fee cancel:', error.response.data);
        setError('Error updating fee cancel: ' + error.response.data.message);
      } else {
        setError('Error updating fee cancel');
        console.error('Error updating fee cancel:', error);
      }
    } finally {
      setLoading(false);
    }
